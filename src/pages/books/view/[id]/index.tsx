...
import { deleteHighlightById } from 'apiSdk/highlights';
...
function BookViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BookInterface>(
    ...
    () =>
      getBookById(id, {
        relations: ['organization', 'user', 'bookmark', 'highlight'],
      }),
  );
  ...
  const highlightHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('bookmark', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/bookmarks/create?book_id=${data?.id}`}>
              ...
              {hasAccess('highlight', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Highlights
                    </Text>
                    <NextLink passHref href={`/highlights/create?book_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.highlight?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/highlights/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.page_number}</Td>
                            <Td>{record.text}</Td>
                            <Td>
                              {hasAccess('highlight', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/highlights/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('highlight', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    highlightHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
